...
- [x] 날짜 기본값 오늘로 설정
- [x] 원정팀 확정 시 경기 데이터 자동 불러오기 (autoFetchGame, 무음 실패)
- [x] 기록 목록 결과별 필터 추가 (전체/승/패/무/중립/취소, RecordListClient)
- [x] RecordCard·상세 페이지 중립 배지 처리 (null 단언 제거, 보라색 배지)
- [x] 홈/원정 승률 카드 '응원경기 기준' 명시
- [x] 대시보드 '내 직관 현황' / '{favoriteTeam} 경기' 두 섹션으로 분리, 제목 '내 직관 통계'로 변경
- [x] 최근 5경기 헤더에 '전체 직관 기준' 설명 추가
- [x] 홈팀·원정팀 중복 선택 방지 (선택된 팀 상대 드롭다운에서 제외)
- [x] 수정 모드에서 중립 경기 응원팀 자동 추론 (result=null → 'neutral')
- [x] 기록 목록 날짜 정렬 토글 (최신순/오래된순) 및 구장 필터 추가
- [x] MonthlyChart → 주차별 직관 횟수로 변경 (weeklyMap, 'N주차' 레이블, 월별 필터 버튼)
- [x] 원정팀 드롭다운 → 읽기 전용 텍스트로 변경 (자동 조회 전용)
- [x] 우천취소 경기 결과 자동 선택 race condition 수정 (isCancelledRef로 비동기 클로저 문제 방지)
